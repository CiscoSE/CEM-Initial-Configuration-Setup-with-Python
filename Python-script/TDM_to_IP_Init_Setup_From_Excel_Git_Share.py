...
# ----------------------------------------------------------------------------------------------------------------------------------------------------------
###************ASR 9K/NCS 4K/NCS 55xx:(IOS-XR Router) Setup**********####

eRow = eRow + 1
eColumn = 2

No_of_9K_Nodes = sheetN.cell(row=eRow, column=eColumn).value
print("Number of ASR 9K/ NCS 4K/NCS 55xx in the Topology: {}".format(No_of_9K_Nodes))

if (No_of_9K_Nodes <= 0):
    print("No ASR 9K/NCS 4K/NCS 55xx for Setup\n")
else:
    print("\n*********Starting ASR 9K/NCS 4K/NCS 55xx Setup***********")

    for count in range(1, No_of_9K_Nodes + 1):
        tn2 = telnetlib.theNULL
        eRow = eRow + 1
        eColumn = 3

        tn2 = login_to_host(tn2, "Setup for ASR 9K/NCS 4K/NCS 55xx Node {}".format(count))

        MPLS_label_range_info = "MPLS label range<16000-1048575>"
        RSVP_bandwidth_input_info = " Enter rsvp bandwidth( use gbps with a space) in Number or Percentage(use % with input): "
        # print(obtain_network_info(MPLS_label_range_info, RSVP_bandwidth_input_info))

        LoopbackIp, router_ospf_process_id, No_of_interfaces, int_list, \
        ip_list, subnet_list, defaultMplsLabelRangeCheck, setRSVPBandWidthString, \
        minLabel, maxLabel = obtain_network_info(IOS_XR, MPLS_label_range_info, RSVP_bandwidth_input_info)

        interface_string = ''
        for i in range(1, No_of_interfaces + 1):
            interface_string = interface_string + 'interface {}'.format(int_list[i-1]) + '\n'
        ...
        for ints in range(1, No_of_interfaces + 1):
            tn2.write((
                     "int %s\n" %int_list[ints-1] +
                     "bandwidth " + setRSVPBandWidthString[ints-1] + "\n" ).encode('ascii'))
